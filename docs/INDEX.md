...
| [issue-17-clone-ssh-DESIGN.md](issue-17-clone-ssh-DESIGN.md) | Fix for #17 / architecture R1 (Workspace-Access): clone via the managed SSH credential — `git clone` uses the SSH URL (reusing #15's `https_github_to_ssh`) when GitHub-HTTPS and the mounted key is present, so private repos clone (clone+push unified on one credential path). Idempotent backstop unchanged; credential-identity (OAQ-3) out of scope. |
